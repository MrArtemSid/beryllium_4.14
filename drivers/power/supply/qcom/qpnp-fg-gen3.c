...
static int fg_set_cycle_count(struct fg_gen3_chip *chip, int value)
{
	int rc = 0;
	int i = 0;
	u8 data[2];
	...
	for (i = 0; i < BUCKET_COUNT; i++) {
		...
		rc = fg_sram_write(&chip->fg, CYCLE_COUNT_WORD + (i / 2),
				CYCLE_COUNT_OFFSET + (i % 2) * 2, data, 2,
				FG_IMA_DEFAULT);
		if (rc < 0)
			pr_err("failed to write BATT_CYCLE[%d] rc=%d\n",
...
static void profile_load_work(struct work_struct *work)
{
	...
	if (!is_profile_load_required(fg)) {
		...
		if (vbatt_predict_uv - vbatt_uv > VBATT_RANGE_DISCHARGE_UV ||
			...
			fg_dbg(fg, FG_SOMC, "SOC is ready\n");
		}
		fg->profile_load_status = PROFILE_LOADED;
		goto done;
	}
...
{	struct fg_dev *chip = container_of(work,
				struct fg_dev,
				soc_work.work);
...
static void empty_restart_fg_work(struct work_struct *work)
{
	...
	if (!usb_present) {
		if (chip->profile_load_status == PROFILE_LOADED) {
			...
			rc = fg_restart(chip, SOC_READY_WAIT_TIME_MS);
			if (rc < 0) {
				pr_err("Error in restarting FG, rc=%d\n", rc);
...
static int fg_somc_set_batt_aging_level(struct fg_dev *fg, const char *buf)
{
	int rc, msoc;

	rc = kstrtoint(buf, 10, &fg->batt_aging_level);
	if (rc < 0) {
			...
			const char *buf, size_t count)
{
	...
	switch (off) {
	case ATTR_BATT_AGING_LEVEL:
		rc = fg_somc_set_batt_aging_level(fg, buf);
		if (rc < 0)
			count = 0;
